...
export default function StudentsPage() {
    ...
    const fetchStudents = useCallback(async () => {
        try {
            ...
            setStudents(data);
        } catch (err) { showToast(err instanceof Error ? err.message : "Failed to load students", "error"); }
            ...
            {showModal ? (
                <>
                    <div className="page-header">
                        <div>
                            <h1 className="page-title">{editing ? "Edit Student" : "New Student"}</h1>
                            <p className="page-subtitle">{editing ? "Update student details" : "Add a new student record"}</p>
                        </div>
                    ...
                    <div className="glass-card" style={{ maxWidth: 640, margin: "0 auto", padding: 32 }}>
                        <form onSubmit={handleSubmit}>
                            ...
                            <div style={{ display: "flex", gap: 12, justifyContent: "flex-end", marginTop: 32, paddingTop: 20, borderTop: "1px solid var(--border-light)" }}>
                                ...
                                <button type="submit" className="btn btn-primary" style={{ padding: "10px 28px" }}>{editing ? "Update Student" : "Create Student"}</button>
                            </div>
                        </form>
                    </div>
                </>
